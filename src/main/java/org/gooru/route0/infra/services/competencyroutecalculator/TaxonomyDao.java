...
interface TaxonomyDao {
    ...
    @SqlQuery("SELECT dcmt.tx_subject_code, dcmt.tx_domain_code, dcmt.tx_comp_code, dcmt.tx_comp_seq FROM   "
                  + " domain_competency_matrix_tx dcmt WHERE  dcmt.tx_subject_code = :subjectCode AND "
    ...
    @Mapper(CompetencyMapper.class)
    @SqlQuery("SELECT tx_subject_code, tx_domain_code, tx_comp_code, tx_comp_seq FROM domain_competency_matrix_tx "
                  + " WHERE (tx_subject_code, tx_domain_code, tx_comp_seq) IN (SELECT dcmt.tx_subject_code, "
                  + " dcmt.tx_domain_code, Max(dcmt.tx_comp_seq) tx_comp_seq FROM domain_competency_matrix_tx dcmt "
    ...
    List<Competency> fetchProficiencyForUserInSpecifiedSubjectAndDomains(@Bind("userId") String userId,
        @Bind("subjectCode") String subjectCode, @Bind("domainCodes") PGArray<String> domainCodes);

    @Mapper(DomainModelMapper.class)
    @SqlQuery("select tx_subject_code, tx_domain_code, tx_domain_name, tx_domain_seq from domains_tx where "
                  + "tx_domain_code = any(:domains) and tx_subject_code = :subjectCode order by tx_domain_seq")
    List<DomainModel> fetchDomainModelsForSpecifiedDomainsInOrder(@Bind("subjectCode") String subjectCode,
    ...
    @SqlQuery("SELECT tx_subject_code, tx_domain_code, tx_comp_code, tx_comp_name, tx_comp_desc,"
                  + " tx_comp_student_desc, tx_comp_seq FROM domain_competency_matrix_tx WHERE "
                  + " tx_subject_code = :subjectCode AND tx_domain_code = any(:domains) ORDER BY "
                  + " tx_domain_code, tx_comp_seq")
    List<CompetencyModel> fetchDomainCompetencyMatrixForSpecifiedDomains(@Bind("subjectCode") String subjectCode,
        @Bind("domains") PGArray<String> domains);
}
