...
interface ContentRoutePersisterDao {
  ...
  @GetGeneratedKeys
  @SqlUpdate(
      "insert into user_route0_content(user_id, class_id, course_id, status, route0_content, "
  ...
  long persistRoute0Content(@BindBean ContentRouteInfo info, @Bind("status") String status,
      ...
      @Bind("userCompetencyRoute") String userCompetencyRoute);

  @SqlBatch(
      "insert into user_route0_content_detail (user_route0_content_id, unit_id, unit_title, unit_sequence, "
  ...
  List<UserRoute0ContentDetailModel> fetchDetailModelsForSpecifiedRoute0(
      @Bind("contentRouteId") long contentRouteId);

  @SqlUpdate("update user_route0_content set route0_content = :route0Content::jsonb where id = :id")
  void updateRouteInfoForRoute0Content(@Bind("id") Long id,
      @Bind("route0Content") String route0Content);
}
