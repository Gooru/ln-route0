...
interface SuggestionProviderDao {
    ...
    @SqlQuery("select filt.competency, filt.content_type, filt.item_id from (select competency, content_type, "
                  + " item_id, row_number() over (partition by competency, content_type  order by weight desc) from "
                  + " competency_content_map where competency = any(:competencies)) filt where  row_number < 2")
