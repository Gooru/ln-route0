...
interface SuggestionProviderDao {
  ...
  @SqlQuery("select filt.competency, filt.content_type, filt.item_id from (select competency, content_type,  "
      ...
      + " content_type desc) from  competency_content_map where competency = any(:competencies) and primary_language = :primaryLanguage) filt "
      + " where row_number < 2")
  List<SuggestedItem> fetchSuggestionsForCompetencies(
      @Bind("competencies") PGArray<String> competencies,
      @Bind("primaryLanguage") Integer primaryLanguage);
