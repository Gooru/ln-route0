 ...
 * <p>
 * Do not allow update if current status is NA. Also if user has already accepted it, they cannot
 * reject it. However, if it is pending user can accept or reject it. If it is rejected, user can
 * accept it. As an optimization, if the status value in DB is same as user provided value, do not
 * update db. Just short circuit and return success.
 *
 * @author ashish.
 */
class AcceptRejectRoute0Service {
  ...
  private void updateRoute0Status() {
    if (route0Status.equals(command.getStatus())) {
      LOGGER.info(
          "Accepting or rejecting already accepted or rejected Route0 respectively. Doing nothing.");
      return;
    }
