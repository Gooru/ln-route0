...
public class CalculateCompetencyRouteProcessor implements AsyncMessageProcessor {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(CalculateCompetencyRouteProcessor.class);
    private CalculateCompetencyRouteService calculateCompetencyRouteService =
        new CalculateCompetencyRouteService(DBICreator.getDbiForDefaultDS(), DBICreator.getDbiForDsdbDS());
    ...
    public Future<MessageResponse> process() {
        vertx.<MessageResponse>executeBlocking(future -> {
            try {
                ...
                CalculateCompetencyRouteCommand command = CalculateCompetencyRouteCommand.builder(eventBusMessage);
                JsonObject competencyRoute = calculateCompetencyRouteService.calculateCompetencyRoute(command);
